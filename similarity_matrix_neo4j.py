"""
    @Project: PetroSpear
    @File   : similarity_matrix_neo4j.py
    @Author : mulder
    @E-mail : c_mulder@163.com
    @Date   : 2026-09-03
    @Info   : To get rock node embedding vectors of the PGKG.
"""
...
def build_similarity_matrix_by_neo4j(dataset_name):
    ...
    if os.path.exists(matrix_file_name_dir):
        ...
        try:
            # graph = Graph("bolt://localhost:7687", user="XXXX", password="XXXXXXXXX", name="XXXX")
            graph = Graph('bolt://localhost:7687/', auth=("XXXX", "XXXXXXXXX"))
